...
ll rangexor(ll segtree[], int vertex, int start, int end, int l, int r){
    if(l>r) return 0;
    if(start == l and end == r) return segtree[vertex];
    int mid = (start+end)/2;
    ll sum1 = rangexor(segtree, 2*vertex, start, mid, l, min(r, mid));
    ll sum2 = rangexor(segtree, 2*vertex+1, mid+1, end, max(mid+1, l), r);
    return sum1 ^ sum2; 
}
...
int main(){
    ...
    for(int i=0;i<q;i++){
    	int check,a,b;
        cin >> a >> b;
        cout << rangexor(segtree,1,0,n-1,a-1,b-1) << endl;
    }
    return 0;
}